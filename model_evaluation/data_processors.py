...
def read_wic_tsv_test(file_path: Path,
                      tgt_column=0,
                      tgt_ind_column=1,
                      cxt_column=2):
    targets = []
    contexts = []
    target_inds = []
    examples_path = next(file_path.glob('*_examples.txt'))
    ...
    for cxt, t_ind, tgt in zip(contexts, target_inds, targets):
        if not cxt.split(' ')[t_ind].lower().startswith(tgt[:-1]):
            assert False, (tgt[:-1], t_ind, cxt.split(' '), cxt.split(' ')[t_ind].lower())
